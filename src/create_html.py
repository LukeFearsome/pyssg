...
def recursive_page_generation(src,tmpl,dst):
    for item in os.listdir(src):
        if item.endswith(".md"):
            generate_page(os.path.join(src,item),tmpl,os.path.join(dst,item.replace(".md",".html")))
        if os.path.isdir(os.path.join(src,item)):
            os.mkdir(os.path.join(dst,item))
            recursive_page_generation(os.path.join(src,item),tmpl,os.path.join(dst,item))
